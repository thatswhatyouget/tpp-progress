...
function createChart(data: Run[]) {
	...
	data.forEach(run=> {
		...
		if (width < runWidth) width = runWidth;
...
function drawEvent(eventInfo: Event) {
	...
	if (eventInfo.Time) eventImg.title += ": " + eventInfo.Time;
	if (eventInfo.Attempts) eventImg.title += " (" + eventInfo.Attempts + " Attempts)";
