...
module TPP.Display.RunStatus {
    ...
    export function RenderRunStatus(run: TPP.Run, dex: TPP.Pokedex.GlobalDexBase = null) {
        ...
        fetchRunStatus().then(status => {
            ...
            $container.append(DrawItems(status.items, undefined, TMs[run.BaseGame]));
            $container.append(DrawBadges(run));
            $container.append(DrawItems(status.pc_items, run.HostName + "'s PC", TMs[run.BaseGame]));
            if (dex) {
    ...
    function DrawItems(items: TPP.Tv.Item[], title = "Items", itemSupplement: TMList = {}) {
        items = items || [];
