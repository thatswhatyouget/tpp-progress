...
module TPP.Display.RunStatus {
    ...
    function drawRunStatus(run: TPP.Run, dex: TPP.Pokedex.GlobalDexBase, status: TPP.Tv.RunStatus, $container: JQuery) {
        ...
        if (extractBattleFrontier(run).length > 0)
            $container.append(DrawBadges(run, extractBattleFrontier(run)));
    ...
    function extractBadges(run: TPP.Run) {
        return run.Events.filter(e => e.Group == "Badges" || e.Group == "Kingdoms" || e.Group == "Plates"  || e.Group == "Stamps" );
    }
    ...
    function extractBattleFrontier(run: TPP.Run) {
        return run.Events.filter(e => e.Group == "Symbols" || e.Group == "Prints");
    }
