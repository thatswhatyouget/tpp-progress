...
function generateGlobalDex(tppData) {
    ...
    dexSummarize(tppData).then(function (summaries) {
        ...
        if (!skipCheckOwnership) {
            element.prepend("<h2 class='total'>Owned: <span>" + Object.keys(fullList).length + "/" + (PokeList.filter(function (p) { return !!p && p != "MissingNo."; }).length) + "</span></div>");
        }
        else {
            element.prepend("<br>");
        }
