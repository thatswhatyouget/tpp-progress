...
function generatePokedexSummary(tppData) {
    ...
    var $table = $("<table>").append($("<thead>").append($("<tr>").append($("<th>").text("Pokemon")).append(summaries.map(function (s) { return $("<th>").text(s.Run.RunName).css({ 'background-color': s.Run.ColorPrimary, 'border-color': s.Run.ColorSecondary }); }))));
...
function generateGlobalDex(tppData) {
    ...
    return $("<div>").append(PokeList.map(function (p, i) {
        ...
        $entry.attr('data-owned-by', ownedBy.join(' '));
        return $entry;
    }));
}
