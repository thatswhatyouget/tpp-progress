function Scrape(run) {
    $('.doItLive').text('Live');
    var deferred = $.Deferred(), durationExp = /(?:(\d*)w)? *(?:(\d*)d)? *(?:(\d*)h) *(?:(\d*)m) *(?:(\d*)s)?/im, attemptsExp = /Attempt[^\d]*(\d*)/i, promise = deferred.promise();
    if (run.Scraper.url == "http://twitchplayspokemon.org/")
        promise = TppOrgApi(run, deferred);
    else {
        $.ajax({
    ...
    return promise.then(function (r) { return r; }, function (e) { return $.when(run); });
}
