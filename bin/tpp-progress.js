...
function applyScale(ppd) {
    ...
    $(".progressChart .run").forEach(function (run) {
        ...
        if (run.getAttribute('data-duration'))
            run.style.width = Duration.parse(run.getAttribute('data-duration')).TotalTime(scale) * ppd + "px";
...
function staggerStackedRuns(runs, runHeight) {
    ...
    runs.forEach(function (r, i) {
        ...
        if (runs[i - 1]) {
            var thisStart = Duration.parse(runs[i - 1].getAttribute('data-time')).TotalSeconds, thisEnd = thisStart + Duration.parse(runs[i - 1].getAttribute('data-duration')).TotalSeconds;
...
function staggerStackedEvents(events, runHeight) {
    var dir = .15;
